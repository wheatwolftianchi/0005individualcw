...
class LondonRailwayMapper(AbstractLondonRailwayMapper):
    ...
    def newRailwayLine(self, inputList):
        ...
        while len(inuse)>0:
            ...
            mindist = 1145141919810
            cache = current
            for stationName in inuse:
                stationsds = self.railwayNetwork.getStation(stationName)
                dist = self.railwayNetwork.getStation(current).getDistance(stationsds)
                if dist < mindist:
                    cache = stationName
                    mindist = dist
            totaldis += mindist
            visited.append(cache)
            count += 1 
            inuse.remove(cache)
        outputList = visited
        print(totaldis)
